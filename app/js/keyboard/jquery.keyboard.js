...
*/
var lastKeyPressed=[2,7];
/*jshint browser:true, jquery:true, unused:false */
var lastKeyPressed=null; // to track last postion for keyboard position
// ZOE-32242: This flag was not defined outside of this file
...
//            }
        ...
        base.startup = function () {
            ...
            base.$allKeys = base.$keyboard.find('span.ui-keyboard-button');
...
//                    }
                    base.checkMaxLength();
					if(isDefined(KEY_CODES.NON_SUPPORTED_KEYS_ARRAY)){
					e.preventDefault();
					e.stopPropagation();
					}
                })
                .bind('keyup.keyboard', function (e) {
                    ...
                    switch (e.which) {
                        ...
                        case KEY_CODES.RBIKEYBOARD_ESC:
                            base.destroy();
                            base.close();
                            return false;
                    }
                        ...
                        $this
                            ...
                            .attr('title', function (i, t) {
                                // show mouse wheel message
                            	 lastKeyPressed[0] = parseInt($this.attr('data-pos')[0]);
                                 lastKeyPressed[1] = parseInt(($this.attr('data-pos')[2])+($this.attr('data-pos')[3]));
...
//            }
            if((txt === 'nxt' && val.length===pos.start) == false ){
            if (txt === 'prv') {
                t = pos.start - 1;
                txt = (txt === 'prv') ? '' : txt;
                scrL += parseInt(base.$preview.css('fontSize'), 10) * (txt === '<' ? -1 : 0);
            }
        ...
        base.showKeySet = function (el) {
            ...
            if (!base.shiftActive) {base.capsLock = false; 
				$('.ui-keyboard-shift').removeClass('ui-state-default');
				$('.ui-keyboard-shift').addClass('blue-active');
			}
            // check meta key set
            if (base.metaActive) {
...
//                } else
        ...
        base.getLayers = function (el) {
            ...
            keys = el.closest('.ui-keyboard').find('span[data-pos="' + key + '"]').map(function () {
        ...
        base.switchInput = function (goToNext, isAccepted) {
            if (typeof o.switchInput === "function") {
                ...
                var kb, stopped = false,
                    all = $('span, input, textarea, a').filter(':visible'),
        ...
        base.close = function (accepted) {
            if (base.isVisible()) {
                ...
                if (!o.alwaysOpen) {
                    base.$keyboard.hide();
					$("#key-bg").remove();// Code added for keyboard clicking focus lost
					base.destroy();
                }
                if (!base.watermark && base.el.value === '' && base.inPlaceholder !== '') {
                    base.$el
            ...
            var modelName = $('#' + base.el.id).attr('ng-model');
            //console.log(currentFormFieldObj);
            var scope = angular.element(base.el).scope();
            scope.$apply(function () {
                console.log("Evaluating: " + "scope." + modelName + "=\"" + base.el.value + "\";");
                eval("scope." + modelName + "=\"" + base.el.value + "\";");
            });
...
//                }
        ...
        base.buildKeyboard = function () {
            ...
            container = $('<div id = "keyboard-button" />')
                        ...
                        for (key = 0; key < keys.length; key++) {
                            ...
                            if (/^\{\S+\}$/.test(keys[key])) {
                                ...
                                switch (action) {
                                    ...
                                    case 'b':
                                    case 'bksp':
                                        base.addKey('bksp', action).addClass(o.css.buttonAction);
                                        break;
                                    ...
                                    case 'cancel':
                                        base
                                            ...
                                            .addClass(o.css.buttonAction).addClass(o.css.buttonAction);
                                        break;
                                    ...
                                    case 's':
                                    case 'shift':
                                        base.addKey('shift', action).addClass(o.css.buttonAction);
                                        break;
        ...
        base.destroy = function () {
            ...
            base.$el
                ...
                .unbind(unb + '.keyboard')
                .removeData('keyboard');
            //KeyboardOccurrenceFlag = false;
            KeyboardFocus = true;
            setTimeout(function(){
            	helper.SetFocus($.keyboardRefHolder.el.id);
            	KeyboardFocus = false;
           },600);
    ...
    $.keyboard.keyaction = {
        accept: function (base) {
            ...
            base.destroy();
			$("#key-bg").remove();
            return true;     // return false prevents further processing
        },
        ...
        enter: function (base, el, e) {
            ...
            if (tag === 'TEXTAREA' && $(e.target).closest('span').length) {
        ...
        shift: function (base, el) {
            ...
            base.showKeySet(el);
				...
				if (base.navigation_options.position[0] == 3 && base.navigation_options.position[1] == 0 ){
				$('.ui-keyboard-shift').removeClass('ui-state-hover');
				   $('.ui-keyboard-keyset:visible').find('.ui-keyboard-button[data-pos="' + base.navigation_options.position[0] + ',' + base.navigation_options.position[1] + '"]').addClass('ui-state-hover');
				}
				else if (base.navigation_options.position[0] == 3 && base.navigation_options.position[1] == 14) {
				$('.ui-keyboard-shift').removeClass('ui-state-hover');
					$('.ui-keyboard-keyset:visible').find('.ui-keyboard-button[data-pos="' + base.navigation_options.position[0] + ',' + base.navigation_options.position[1] + '"]').addClass('ui-state-hover');
				}
 ...
 //Method to suport remote key numpad keypress & Hot key function
        
        customKeyPress : function(key){
        	var key = event.keyCode;
            
            // Hot key implementation RBIKEYBOARD_ok_F2/RBIKEYBOARD_Backspace_F4/RBIKEYBOARD_spacebutton_F7 these will be removed moving forword
        	if(key == KEY_CODES.NON_SUPPORTED_KEYS_ARRAY[0] || key == KEY_CODES.RBIKEYBOARD_ok_F2 ){
            		$.keyboard.keyaction.accept($.keyboardRefHolder);
        			$.keyboard.keyaction.bksp($.keyboardRefHolder);
        }
			
        	if(key == KEY_CODES.SQUARE || key == KEY_CODES.RBIKEYBOARD_Backspace_F4)
            		$.keyboard.keyaction.bksp($.keyboardRefHolder);
                
        	if(key == KEY_CODES.TRIANGLE || key == KEY_CODES.RBIKEYBOARD_spacebutton_F7)
            		$.keyboard.keyaction.space($.keyboardRefHolder);
        	if(!navigator.userAgent.match('Mozilla')  || !navigator.userAgent.match('Chrome')|| !navigator.userAgent.match('Safari')){    
        	//Do not remove this will work on this
        	if(key == KEY_CODES.RBIKEYBOARD_0)
        		$.keyboardRefHolder.insertText("0");
        	
        	if(key == KEY_CODES.RBIKEYBOARD_1){
        		$.keyboardRefHolder.insertText("1");
        	}
        	
        	if(key == KEY_CODES.RBIKEYBOARD_2){
        		$.keyboardRefHolder.insertText("2");
        	}
        	
        	if(key == KEY_CODES.RBIKEYBOARD_3)
        		$.keyboardRefHolder.insertText("3");
        	
        	if(key == KEY_CODES.RBIKEYBOARD_4)
        		$.keyboardRefHolder.insertText("4");
        	
        	if(key == KEY_CODES.RBIKEYBOARD_5)
        		$.keyboardRefHolder.insertText("5");
        	
        	if(key == KEY_CODES.RBIKEYBOARD_6)
        		$.keyboardRefHolder.insertText("6");
        	
        	if(key == KEY_CODES.RBIKEYBOARD_7)
        		$.keyboardRefHolder.insertText("7");
        	
        	if(key == KEY_CODES.RBIKEYBOARD_8)
        		$.keyboardRefHolder.insertText("8");
        	
        	if(key == KEY_CODES.RBIKEYBOARD_9)
        		$.keyboardRefHolder.insertText("9");

			//to restrict unused keys of TV remote when keybord is in front
			if(isDefined(KEY_CODES.NON_SUPPORTED_KEYS_ARRAY)){
					var index  = KEY_CODES.NON_SUPPORTED_KEYS_ARRAY.map(function(element){return element}).indexOf(key);
					if(index>=0){
						$.keyboardRefHolder.insertText("");
						return;
					}
			}
        }
    ...
    $.keyboard.layouts = {
        'qwerty': {
            'default': [
				'{accept} ` 1 2 3 4 5 6 7 8 9 0 - = {accept}',
				'{bksp} q w e r t y u i o p [ ] \\ {bksp} ',
				'{clear} {empty} a s d f g h j k l ; \' {empty} {clear}',
				'{shift} {empty} {empty} z x c v b n m , . / {empty} {shift}',
				'{cancel} @ .com .net  {space} {prev} {empty} {next} {cancel}'
            ],
            'shift': [
				'{accept} ~ ! @ # $ % ^ & * ( ) _ + {accept}',
				'{bksp} Q W E R T Y U I O P { } | {bksp}',
    ...
    $.keyboard.defaultOptions = {
        ...
        display: {
            ...
            'clear': 'Clear:Clear',             // clear num pad
            ...
            'shift': 'Shift / #?!:Shift', // Updated Text on shift button 
